...
Covered fully in [Hooks](08-hooks.md).
...
Mitigations: deny rules on `rm -rf`, `git push --force`, `git reset --hard`, `DROP TABLE`. A `PreToolUse` hook on `Bash` that pattern-matches. The confirmation-gate prompt from [Everyday workflows](03-everyday-workflows.md). And most fundamentally: **work in git, commit often.**

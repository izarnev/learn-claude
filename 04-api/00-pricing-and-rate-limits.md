...
1. **Prompt caching.** Cache a large, stable prefix (system prompt, documents, tool definitions) and reads against it cost a fraction of normal input. For agents that resend the same context every turn, this is often a 70–90% saving. See [Prompt caching](07-prompt-caching.md).
2. **Batch API.** Submit work asynchronously for a substantial discount. Perfect for evals, bulk classification, and offline processing. See [Streaming and batch processing](09-streaming-and-batch.md).
...
Covered in [Enterprise and governance](../06-production/05-enterprise-and-governance.md).
...
- [Cost and latency optimisation](../06-production/03-cost-and-latency.md)
